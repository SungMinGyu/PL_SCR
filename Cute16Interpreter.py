...
class CuteType:
    ...
    KEYWORD_LIST=('define', 'lambda', 'cond','quote', 'not',
                  'car', 'cdr', 'cons', 'atom?', 'null?', 'eq?' )
...
class CuteInterpreter(object):
    ...
    def run_arith(self, arith_node):
        ...
        if arith_node.type is TokenType.PLUS :
            ...
            if int(rhs1.value) > int(rhs2.value):   return Node(TokenType.TRUE)
            else:   return Node(TokenType.FALSE)
    ...
    def run_func(self, func_node):
        ...
        if func_node.type is TokenType.CAR:
            ...
            if expr_rhs1.type is TokenType.LIST and expr_rhs1.value.type is TokenType.QUOTE:
    ...
    def lookupTable(self, id):
        global tokenTable
        if id.value in tokenTable:
            return tokenTable[id.value]
        else:
            return None
